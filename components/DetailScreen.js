...
const DetailScreen = ({route, navigation}) => {
          ...
          try {
            ...
            if (result === RESULTS.DENIED) {
              ...
              if (requestResult !== RESULTS.GRANTED) {
                Alert.alert(
                  'Permission Required',
                  'Buzzo needs storage access to manage your photos. Please grant permission in your device settings to delete or manage photos.',
                  [
                    {text: 'Cancel', style: 'cancel'},
                    {text: 'Open Settings', onPress: openSettings},
                  ],
              ...
              Alert.alert(
                'Permission Required',
                'Buzzo needs storage access to manage your photos. Please enable storage access in Settings to delete photos.',
                [
                  {text: 'Cancel', style: 'cancel'},
                  {text: 'Open Settings', onPress: openSettings},
                ],
        ...
        if (Platform.Version >= 30) {
          try {
            ...
            if (manageStorageResult === RESULTS.DENIED) {
              Alert.alert(
                'Additional Permission Required',
                'Buzzo requires "All files access" permission on Android 11+ for photo deletion. This will redirect you to settings where you can grant this permission.',
                [
                  {text: 'Cancel', style: 'cancel'},
                  {
                    text: 'Open Settings',
  ...
  const deleteUnstarredPhotos = async () => {
    try {
      ...
      if (!hasPermission) {
        Alert.alert(
          'Permission Required',
          'Buzzo needs storage permissions to delete photos. Please grant the required permissions in your device settings.',
        );
        return;
      }
      ...
      Alert.alert(
        'Error', 
        'Buzzo encountered an error while trying to delete photos. Please try again or check your device permissions.'
      );
    }
  };
  ...
  const keepAllPhotos = async () => {
    ...
    Alert.alert(
      'All Photos Kept',
      'All photos remain in your device gallery, but unstarred ones are now hidden in Buzzo.',
    );
  };
